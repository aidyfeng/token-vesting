...
pub mod tokenvesting {
    ...
    pub fn claim_tokens(ctx: Context<ClaimToken>, _company_name: String) -> Result<()> {
        ...
        let vesting_total_time = employee_account
            .end_time
            .saturating_sub(employee_account.start_time);

        if vesting_total_time == 0 {
        ...
        let vested_amount = if now > employee_account.end_time {
            ...
            {
                Some(product) => product / (vesting_total_time as u64),
        ...
        let signer_seeds: &[&[&[u8]]] = &[&[
            ...
            &[ctx.accounts.vesting_account.treasure_bump],
        ]];
